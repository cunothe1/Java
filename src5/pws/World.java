...
public class World {
	...
	public void unitAdd(String type, int team) {
		...
		unit[unitNew].setInfo(15, unitNew);
		unit[unitNew].setInfo(12, 1);
		unit[unitNew].setInfo(11, team);
		unit[unitNew].setInfo(8, 0);
		unit[unitNew].setInfo(2, 0);
		unit[unitNew].setWorld(this);
	}
	...
	public void unitPlace(int ID, int x, int y) {
		if (unit[ID].getInfo(9)==300) return;
		if (unit[ID].getInfo(10)==300) return;
		unitAt[unit[ID].getInfo(9)][unit[ID].getInfo(11)]=0;
		unit[ID].setX(x*50+110);
		unit[ID].setInfo(9,x);
		unit[ID].setY(y*50-40);
		unit[ID].setInfo(10,y);
		unitAt[x][y]=ID;
	}
	public String getTile(int a, int b) {
		return tile[a][b];
	}
	//=================================================================================================================
	//part 2 Restart
	...
	public void restart() {
    	...
    	unitAdd("archer",1);
    	unitAdd("Swordsman",2);
		if (frame.selectA==null)frame.selectA=unit[1];
		if (frame.selectB==null)frame.selectB=unit[1];
		unitPlace(0,300,300);
		unitPlace(1,2,2);
		unitPlace(2, 12, 4);
		frame.selectA=unit[1];
		frame.selectB=unit[1];
		//unitNew = 0;  //The amount of existing objects, used in for loops
		//unit[] = new Characters[255]; //the unitlist with a current cap of 255
		//unitAt[][] = new int [24][16]; //the current unit on a certain tile
